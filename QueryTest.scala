...
object QueryTest extends Assertions {
  ...
  def getErrorMessageInCheckAnswer(
    ...
    def makeLogicalPlan(df: DataFrame): LogicalPlan = {
      val id = Dataset.curId.getAndIncrement()

      val plan = df.queryExecution.commandExecuted
      if (df.sparkSession.conf.get(SQLConf.FAIL_AMBIGUOUS_SELF_JOIN_ENABLED)) {
    ...
    // val isSorted: Boolean = makeLogicalPlan(df).collect {case s: logical.Sort => s}.nonEmpty
